...
public static void main(String[] args) throws ClassNotFoundException, SQLException, IOException, InterruptedException, NameInUseException, org.pql.bot.AbstractPQLBotEx.NameInUseException {
	...
	//input parameters
	  ...
	  int numberOfRuns = Integer.parseInt(args[3]); //3
...
{	
	...
	//indexing with n bots
	for(int botNum=1; botNum<(numberOfBots+1); botNum++)
	{	
		//remove index
		...
		System.out.println("Indexing completed for k: " + (i+1) + " bots: " + botNum + " run: " + (run+1));
	}
	
	//indexed with n bots----------------------------------------------------------
	
	//get results for n bots
	...
	{	
		for(String id : netsExt.elementAt(kf))
		{
			...
			results.add("1v2;"+botNum+";"+(run+1)+";"+(kf+1)+";"+id+";"+numberOfTransitions+";"+numberOfPlaces+";"+numberOfFlowArcs+";"+nodes+";"+category+";"+indextime+";"+indexStartTime+";"+indexEndTime+"\r\n");
...
File Results = writeCSV(results,".\\Ex1v2Results.csv");
